<template>
  ...
  const product = ref({
    id: route.params.id,
    name: 'HIGH WAISTED FLARED STRAIGHT LEG PANT OLIVE GREEN',
    images: [
      '/path/to/image1.jpg',
      '/path/to/image2.jpg',
      '/path/to/image3.jpg',
    ...
    installment: 1316.67,
    code: 'NRI094-01-UK14',
    availability: 'Hurry! Only 4 available',
    fabric: '100% Rayon (non-stretch)',
    color: 'Olive Green',
    sizes: ['UK14', 'UK12', 'UK10', 'UK8', 'UK6'],
    brand: 'NORA REED'
  });
  ...
  const formattedPrice = computed(() => `LKR ${product.value.price}.00`);
  const formattedInstallment = computed(() => `LKR ${product.value.installment} x 3 with Mintpay`);
  </script>
